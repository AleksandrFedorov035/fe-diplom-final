...
export default function Tickets({ loading, setLoading }) {
  ...
  useEffect(() => {
    ...
    if (searchData.startDate) {
      ...
      params.date_start = `${d.getFullYear()}-${String(
        d.getMonth() + 1
      ).padStart(2, "0")}-${String(d.getDate()).padStart(2, "0")}`;
    }
    if (searchData.endDate) {
      const d = searchData.endDate;
      params.date_end = `${d.getFullYear()}-${String(d.getMonth() + 1).padStart(
        2,
        "0"
    ...
    if (searchData.price_from != null)
      params.price_from = searchData.price_from;
