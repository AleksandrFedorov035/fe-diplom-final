...
export default function SearchForm({ isTrainsPage = false }) {
  ...
  const showModal = (type, title, text) =>
    setModal({ visible: true, type, title, text });
  ...
  const validate = () => {
    ...
    if (startDate && endDate && endDate < startDate) {
      showModal(
        "error",
        "Ошибка дат",
        "Дата возвращения не может быть раньше даты поездки"
      );
      return false;
    }
    return true;
            ...
            <div className="direction-section">
              ...
              <div className="cities trains-cities">
                <div className="input-group trains-input">
                  ...
                  />
                  <img src={base + "/geo.png"} alt="geo" />
                </div>
                <img
                  src={base + "/reverse.png"}
                  alt="reverse"
                  className={`reverse trains-reverse ${
                    !fromCityValue || !toCityValue ? "disabled" : ""
                  }`}
                  onClick={reverseValues}
                  title={
                    !fromCityValue || !toCityValue
                      ? "Выберите оба города для смены мест"
                      : "Поменять местами города"
                  }
                  style={{
                    cursor:
                      !fromCityValue || !toCityValue
      ...
      <div className="form-container">
        ...
        <div className="cities">
          ...
          <img
            ...
            style={{
              cursor:
                !fromCityValue || !toCityValue ? "not-allowed" : "pointer",
              opacity: !fromCityValue || !toCityValue ? 0.4 : 0.8,
            }}
          />
          <div className="input-group">
